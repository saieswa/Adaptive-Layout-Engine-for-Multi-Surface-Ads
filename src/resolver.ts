...
function fits(box: Box, placed: Box[], bounds: Box): boolean {
  if (
    box.x < bounds.x ||
    box.y < bounds.y ||
...
export function resolveLayout(spec: AdSpec, surface: SurfaceProfile): ResolvedLayout {
  ...
  const orientation =
    bounds.width / Math.max(bounds.height, 1) > 1.55
      ? "wide"
  ...
  const contentByRole = (role: AdElement["role"]) =>
    elements.find((e) => e.role === role);
  ...
  const active = orderedRoles.filter(Boolean) as AdElement[];

  const makeFont = (element: AdElement) => {
    const base =
      element.type === "button"
        ? 15
...
function repack(
  ...
  if (orientation !== "wide" && result.length) {
    ...
    for (const item of result) item.x += dx;
  }

  return result;
}

function isValid(items: ResolvedElement[], bounds: Box, surface: SurfaceProfile): boolean {
  ...
  for (const item of visible) {
    ...
    if (item.type === "button" && surface.minTapTarget && item.height < surface.minTapTarget) return false;
    if (item.type === "text" && surface.minTextSize && item.fontSize < surface.minTextSize) return false;
  }

  for (let i = 0; i < visible.length; i++) {
    for (let j = i + 1; j < visible.length; j++) {
      if (overlaps(visible[i], visible[j])) return false;
    }
  }
